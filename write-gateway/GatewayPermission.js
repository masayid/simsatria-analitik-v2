...
function gatewayRequireAction_(action, payload) {
  ...
  if(action==='SPREADSHEET_ENSURE_SHEET'){
    ...
    if(sheet!=='KELAS'&&sheet!=='GURU')throw new Error('Gateway hanya mengizinkan pembuatan sheet KELAS atau GURU.');
  ...
  if(action==='SPREADSHEET_READ'){
    const sheet=gatewayClean_(payload.sheet).toUpperCase();
    if(sheet!=='KELAS'&&sheet!=='GURU'&&sheet!=='TRX_AGENDA_GURU')throw new Error('Gateway hanya mengizinkan pembacaan sheet KELAS, GURU, dan TRX_AGENDA_GURU.');
  }
