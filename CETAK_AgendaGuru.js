 ...
 * Logo kop: kolom PREVIEW GAMBAR TANDA TANGAN (IMAGE) bentuk-2.
 * Tanda tangan Kepala Sekolah: prioritas bentuk-1, fallback bentuk-2.
 */
...
function buildCETAKAgendaGuruHtml_(school, guru, ks, start, end, rows) {
  ...
  // Logo kop bersumber dari KEPALA_SEKOLAH -> bentuk-2.
  const logo = clean_(ks['PREVIEW GAMBAR TANDA TANGAN (IMAGE) bentuk-2']);

  // Tanda tangan tetap prioritas bentuk-1, fallback bentuk-2.
  ...
  // DPL, PM, dan Prinsip digabung menjadi satu kolom dengan label tebal.
  const formatDplPmPrinsip = function(r) {
    return '<div><strong>DPL:</strong> '+esc(r.dpl || '-')+'</div>' +
           '<div><strong>PM:</strong> '+esc(r.pengalaman_belajar || '-')+'</div>' +
           '<div><strong>Prinsip:</strong> '+esc(r.prinsip_pembelajaran || '-')+'</div>';
  };
  ...
  return '<!doctype html><html><head><meta charset="UTF-8"><style>'+
    ...
    '</style></head><body>'+
    '<div class="kop">'+(logo?'<img class="logo" src="'+esc(logo)+'">':'<div class="logo"></div>')+'<div class="koptext"><div class="prov">'+esc(provinsi)+'</div><div class="dinas">'+esc(dinas)+'</div><div class="school">'+esc(namaSekolah)+'</div><div class="addr">'+esc(alamat)+'</div></div></div>'+
    ...
    '<table class="data"><thead><tr><th>No</th><th>Tanggal</th><th>Jam ke-</th><th>Kelas</th><th>Tujuan Pembelajaran</th><th>DPL / PM / Prinsip</th><th>Siswa Tidak Masuk</th><th>Materi & Catatan</th></tr></thead><tbody>'+body+'</tbody></table>'+
