...
const SCHOOL2_USERS = Object.freeze([
  {
    id_user: '04',
    nama: 'Guru Sekolah 2',
    email: 'kurikulum.sman3pwr@gmail.com',
    role: 'GURU'
  },
  {
    id_user: '05',
    nama: 'Wali Sekolah 2',
    email: 'wali03@gmail.com',
    role: 'WALI_KELAS'
  },
  {
    id_user: '06',
    nama: 'Karyawan Sekolah 2',
    email: 'karyawan03@gmail.com',
    role: 'KARYAWAN'
  },
  {
    id_user: '07',
...
function setup25_registerSchool2Users() {
  ...
  if (!findSchoolById_(schoolId)) {
    throw new Error('Sekolah sekolah kedua belum terdaftar atau tidak ACTIVE: ' + schoolId);
  }
...
function setup26_checkSchool2Users() {
  ...
  const checks = expected.map(function(item) {
    const found = users.find(function(user) {
      return user.idUser.toUpperCase() === item.idUser.toUpperCase() &&
  ...
  const result = {
    ...
    users: users,
    message: passed
      ? 'Semua user sekolah kedua terdaftar ACTIVE dengan role dan sekolah yang benar.'
      : 'Masih ada user sekolah kedua yang belum sesuai.'
  };
