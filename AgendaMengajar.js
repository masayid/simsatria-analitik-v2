...
function getAgendaUser_() {
  ...
  if (!session || session.ok !== true || !session.data || !session.data.user) throw new Error('Sesi pengguna tidak tersedia.');
...
function simpanAgendaMengajar(data) {
  ...
  data = data || {};
  const tanggal = clean_(data.tanggal), sesi = clean_(data.sesi), kelas = clean_(data.kelas), tujuan = clean_(data.tujuan), materi = clean_(data.materi);
  const dpl = clean_(data.dpl), pengalaman = clean_(data.pengalaman), prinsip = clean_(data.prinsip), rekapTidakMasuk = clean_(data.rekapTidakMasuk), buktiFisik = clean_(data.buktiFisik);
...
function simpanBuktiFisikAgendaMengajar(file) {
  ...
  if (['image/jpeg','image/png','image/webp','image/gif'].indexOf(mimeType) < 0) throw new Error('Bukti fisik hanya boleh berupa gambar JPG, PNG, WEBP, atau GIF.');
  if (Number(file.sizeBytes || 0) > 2*1024*1024 || Math.floor(String(file.base64).length*3/4) > 2*1024*1024) throw new Error('Ukuran bukti fisik maksimal 2 MB.');
  const result = uploadViaGateway(AGENDA_MENGAJAR_MENU,{base64:file.base64,mimeType:mimeType,name:file.name});
  return ok_({fileId:result.data&&(result.data.id||result.data.fileId)||result.id||'',fileName:result.data&&(result.data.name||result.data.fileName)||result.name||file.name,url:result.data&&(result.data.url||result.data.fileUrl)||result.url||''},'Bukti fisik berhasil diunggah.');
}

function getAgendaMengajarFormOptions() {
  requirePermission_(APP_CONFIG.PERMISSION.READ, AGENDA_MENGAJAR_MENU);
  const user = getAgendaUser_();
  let kelas = AGENDA_MENGAJAR_OPTIONS.kelasMock.slice(), sumberKelas = 'MOCK';
  try {
    const result = readSheetViaGateway('KELAS'), data = result && result.data ? result.data : result;
    if (data && data.exists === true) { const fromSheet = extractKelasFromSheet_(Array.isArray(data.values)?data.values:[]); if(fromSheet.length){kelas=fromSheet;sumberKelas='KELAS';} }
  } catch(e) {}
  return ok_({nama:clean_(user.nama),idUser:clean_(user.idUser || user.id_user),sesi:AGENDA_MENGAJAR_OPTIONS.sesi.slice(),dpl:AGENDA_MENGAJAR_OPTIONS.dpl.slice(),prinsip:AGENDA_MENGAJAR_OPTIONS.prinsip.slice(),pengalaman:AGENDA_MENGAJAR_OPTIONS.pengalaman.slice(),kelas:kelas,sumberKelas:sumberKelas});
}

function extractKelasFromSheet_(values) {
  if (!values || !values.length) return [];
  const header=values[0].map(function(v){return clean_(v).toLowerCase();});
  let idx=header.findIndex(function(h){return /^(kelas|nama_kelas|nama kelas|kode_kelas|kode kelas|tingkat)$/.test(h);}), start=1;
  if(idx<0){idx=0;start=0;}
  const seen={},result=[];
  for(let i=start;i<values.length;i++){const value=clean_(values[i][idx]);if(!value)continue;const key=value.toUpperCase();if(seen[key])continue;seen[key]=true;result.push(value);}
  return result;
}
...
function getAgendaMengajarList(tanggalAwal,tanggalAkhir) {
  requirePermission_(APP_CONFIG.PERMISSION.READ,AGENDA_MENGAJAR_MENU);
  const user=getAgendaUser_(), userId=clean_(user.idUser || user.id_user), start=agendaDateKey_(tanggalAwal), end=agendaDateKey_(tanggalAkhir);
  ...
  const result=readSheetViaGateway(AGENDA_MENGAJAR_SHEET), data=result&&result.data?result.data:result;
  if(!data||data.exists!==true||!Array.isArray(data.values)||data.values.length<2) return ok_({rows:[],total:0,nama:clean_(user.nama)});
  const values=data.values, header=values[0].map(function(v){return clean_(v).toLowerCase();}), idx={};
  ...
  for(let i=1;i<values.length;i++){
    const r=values[i]; if(clean_(r[idx.id_user])!==userId) continue;
    const rawDate=cell_(r,idx.tanggal), key=agendaDateKey_(rawDate); if(key<start||key>end) continue;
    rows.push({rowNumber:i+1,timestamp:cell_(r,idx.timestamp),id_user:cell_(r,idx.id_user),nama:cell_(r,idx.nama),tanggal:rawDate,sesi:cell_(r,idx.sesi),kelas:cell_(r,idx.kelas),tujuan_pembelajaran:cell_(r,idx.tujuan_pembelajaran),materi_pembelajaran:cell_(r,idx.materi_pembelajaran),dpl:cell_(r,idx.dpl),pengalaman_belajar:cell_(r,idx.pengalaman_belajar),prinsip_pembelajaran:cell_(r,idx.prinsip_pembelajaran),rekap_siswa_tidak_masuk:cell_(r,idx.rekap_siswa_tidak_masuk),bukti_fisik:cell_(r,idx.bukti_fisik),_sortDate:key});
  }
  rows.sort(function(a,b){return a._sortDate.localeCompare(b._sortDate)||a.rowNumber-b.rowNumber;});
  rows.forEach(function(r){delete r._sortDate;});
  return ok_({rows:rows,total:rows.length,nama:clean_(user.nama)});
}

function cell_(row,idx){return idx>=0?clean_(row[idx]):'';}

function getAgendaRowOwned_(rowNumber){
  const user=getAgendaUser_(), result=readSheetViaGateway(AGENDA_MENGAJAR_SHEET), data=result&&result.data?result.data:result;
  if(!data||data.exists!==true||!Array.isArray(data.values)) throw new Error('Data agenda tidak tersedia.');
  const values=data.values,n=Number(rowNumber); if(!(n>=2)||n>values.length) throw new Error('Baris agenda tidak ditemukan.');
  const header=values[0].map(function(v){return clean_(v).toLowerCase();}), idIdx=header.indexOf('id_user'); if(idIdx<0) throw new Error('Kolom id_user tidak ditemukan.');
  const row=values[n-1], userId=clean_(user.idUser || user.id_user); if(clean_(row[idIdx])!==userId) throw new Error('Agenda bukan milik user aktif.');
  return {user:user,values:values,row:row,rowNumber:n,header:header};
}

function updateAgendaMengajar(data){
  requirePermission_(APP_CONFIG.PERMISSION.INPUT,AGENDA_MENGAJAR_MENU); data=data||{}; const owned=getAgendaRowOwned_(data.rowNumber), user=owned.user, row=owned.row.slice(), header=owned.header;
  ...
  put('nama',user.nama);put('tanggal',data.tanggal);put('sesi',data.sesi);put('kelas',data.kelas);put('tujuan_pembelajaran',data.tujuan);put('materi_pembelajaran',data.materi);put('dpl',data.dpl);put('pengalaman_belajar',data.pengalaman);put('prinsip_pembelajaran',data.prinsip);put('rekap_siswa_tidak_masuk',data.rekapTidakMasuk);if(data.buktiFisik!==undefined)put('bukti_fisik',data.buktiFisik);
  const result=gatewayCall_('SPREADSHEET_UPDATE_ROW',{sheet:AGENDA_MENGAJAR_SHEET,rowNumber:owned.rowNumber,row:row}); return ok_({rowNumber:owned.rowNumber,gateway:result},'Agenda berhasil diperbarui.');
}

function hapusAgendaMengajar(rowNumber){requirePermission_(APP_CONFIG.PERMISSION.INPUT,AGENDA_MENGAJAR_MENU);const owned=getAgendaRowOwned_(rowNumber),result=gatewayCall_('SPREADSHEET_DELETE_ROW',{sheet:AGENDA_MENGAJAR_SHEET,rowNumber:owned.rowNumber});return ok_({rowNumber:owned.rowNumber,gateway:result},'Agenda berhasil dihapus.');}

function cetakAgendaMengajarPdf(tanggalAwal,tanggalAkhir){
  requirePermission_(APP_CONFIG.PERMISSION.PDF,AGENDA_MENGAJAR_MENU); const list=getAgendaMengajarList(tanggalAwal,tanggalAkhir);
  if(!list||list.ok!==true||!list.data||!list.data.rows.length) throw new Error('Tidak ada agenda pada rentang tanggal yang dipilih.');
  const session=getSessionContext(),user=getAgendaUser_(),school=session.data.school||{},namaSekolah=clean_(school.namaSekolah||school.nama_sekolah||'SIM SATRIA');
  const html=buildAgendaPdfHtml_(namaSekolah,user.nama,tanggalAwal,tanggalAkhir,list.data.rows),name='Agenda_Mengajar_'+String(tanggalAwal).replace(/-/g,'')+'_'+String(tanggalAkhir).replace(/-/g,'');
